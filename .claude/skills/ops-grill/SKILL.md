...
5-1 と 5-2 は同じ Section 4「ステップ一覧」を単一の情報源として作図し、内容が食い違わないようにすること。5-2 の `participant` は Section 3 の RACI 表・カテゴリ②で確定したアクター一覧と一致させる。カテゴリ⑤（外部者との情報授受）が「省略」の場合は、5-2 は社内アクターのみで構成し、無理に外部者を登場させない。

**未確定要素の表記規約（5-1・5-2 共通の考え方をシーケンス図向けに翻訳したもの）:**
- 未確認のアクター（存在自体が仮説）: `participant` の表示名に `【?】` を付ける（例: `participant 外部窓口【?】`）
...
- 周期的な締め処理でバリエーションが並行する場合は `par ... and ... end` ブロックで表現する
